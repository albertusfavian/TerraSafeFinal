...
/** @brief Describes tile store usage modes. */
// NOLINTNEXTLINE(modernize-use-using)
typedef NS_CLOSED_ENUM(NSInteger, MBMTileStoreUsageMode)
{
    /**
     * @brief tile store usage is disabled
     *
     * The implementation skips checking tile store when requesting a tile.
     */
    MBMTileStoreUsageModeDisabled,
    /**
     * @brief tile store enabled for accessing loaded tile packs.
     *
     ...
     * falls back to requesting the individual tile and storing it in the ambient cache.
     */
    MBMTileStoreUsageModeReadOnly,
    /**
     * @brief tile store enabled for accessing local tile packs and for loading new tile packs from server.
     *
     ...
     * the ambient cache won’t be used as long as the up-to-date tile pack exists in the cache.
     */
